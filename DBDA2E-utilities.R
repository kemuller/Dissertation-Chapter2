...
HDIofMCMC = function( sampleVec , credMass=0.95 ) {
  ...
  ciIdxInc = ceiling( credMass * length( sortedPts ) ) #upper bound of HDI
  nCIs = length( sortedPts ) - ciIdxInc #lower bound of HDI
  ciWidth = rep( 0 , nCIs )
  for ( i in 1:nCIs ) {
    ciWidth[ i ] = sortedPts[ i + ciIdxInc ] - sortedPts[ i ] # difference between sorted points
  }
