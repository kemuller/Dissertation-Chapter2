...
##compare 1, 5, and 30 years of soy.
hostpres <- phbdata[phbdata$rotation %in% c("C5","S1","S5","Ss"),
                  c("phbEst_zerobound","rotation","shootMassG","plantID","plot","plate","phbslope",
                    "plotrep","plantrep")]

str(hostpres)
hostpres$sqrt.phb <- sqrt(hostpres$phbEst_zerobound) # square-root transformation for skew
hostpres$rotation <- factor(hostpres$rotation,levels=c("C5","S1","S5","Ss"))
hostpres$plot <- factor(hostpres$plot)
hostpres$yearsWithSoy <- hostpres$rotation
levels(hostpres$yearsWithSoy) <- c("0","1","5","30")

# I want plant ID to be a unique number so it's easy to put in order. 
hostpres.pd <- pd[pd$rotation %in% c(c("C5","S1","S5","Ss")),]
hostpres.pd$rotation <- factor(hostpres.pd$rotation, levels= c("C5","S1","S5","Ss") )
hostpres.pd$yearsWithSoy <- hostpres.pd$rotation
levels(hostpres.pd$yearsWithSoy) <- c("0","1","5","30")
hostpres.pd$plot <- factor(hostpres.pd$plot)
hostpres.pd <- hostpres.pd[order(hostpres.pd$rotation,hostpres.pd$plot),]
hostpres.pd$plotnum <- rep(1:16,each=4)
hostpres.pd$plantnum <- c(1:64)
hostpres <- merge(hostpres, hostpres.pd[,c("plantID","plotnum","plantnum")])
# order the dataframe, just to help me wrap my head around stuff
hostpres <- hostpres[order(hostpres$yearsWithSoy,hostpres$plotnum,hostpres$plantnum),]

##format the data to get x,y varible names for the model.
fileNameRoot = "SROChostpres-"
graphFileType="png"
seednum  = 43
...
source("bayesScript_rotationPlotPlant.R", echo=T) #model is commented out so it doesn't re-run
load("SROChostpres-Mcmc.Rdata")
load("SROChostpres-priors-Mcmc.Rdata")
...
#--------------------------------------
##Diagnostics:
##Chains all look well mixed in diagnostic plots
postpars <- read.csv("SROChostpres-SummaryInfo.csv",stringsAsFactors = F)
postpars <- postpars[1:length(parameters),1:8]
##Autocorrelation:
postpars[postpars$ESS<10000,]
## There's slight autocorrelation in within-group sd for group 2. But ESS is close to 10,000.
# There's more autocorrelation in hyperparamters for ySigma, but that's OK.
...
colnames(x1means.sim) <- levels(hostpres$yearsWithSoy)
...
colnames(x2means.sim) <- unique(hostpres$plotnum)
...
x3forplotting <- data.frame(meanest.plant = meanofx3means.sim,HDIlow.y =ysim.HDIlow.plant,
                            ...
                            meanfromdata.plant = tapply(hostpres$sqrt.phb,hostpres$plantnum,mean),
...
hostpres <- merge(hostpres, x3forplotting[,c("plantnum","rank.plantmean")],by="plantnum")

# compare simulated to actual data for all factor levels
#tiff(file="SROChostpresPostPredCheck1.tiff", width=1000, height=550, res=100)
# plot plants in order of mean to help visualization
xlim=c(1,Nx3Lvl)
par(mar=c(5.1,4.1,4.1,2.1) + c(0,+1,-2,0) )
plot(hostpres$rank.plantmean, hostpres$sqrt.phb,xlim=xlim,col=grey(0,alpha=0.5),xlab="plantID (sorted by mean PHB)",
...
#tiff("SROChostpresPostPredCheck2.tiff", width=600, height=550, res=100)
...
for(i in 1:Ntotal){
  hostpres$yhat[i] <- b0 + b1[x1[i]] + b2[x2[i]] + b3[x3[i]]
}
hostpres$residual <- hostpres$sqrt.phb - hostpres$yhat
...
#tiff("SROChostpres_QQresid.tiff", width=800, height = 680, res=100)
layout(matrix(1:4,ncol=2,byrow=T))
for(i in 1:4){
  r <- hostpres[hostpres$yearsWithSoy==x1levels[i],"residual"]
...
## check if calibration error explains residual variance:
plot(hostpres$phbslope,hostpres$residual,xlab="calibration slope",ylab="residuals")
abline(h=0,lty=2)
calbyplant <- tapply(hostpres$phbslope, hostpres$plantnum,mean)
plot(calbyplant,x3resid ,ylab="residuals by plant mean",xlab="calibration slope")
abline(h=0,lty=2)

#tiff("SROChostpres_QQnorm.tiff", width=800, height = 680, res=100)
layout(matrix(1:4,ncol=2,byrow=T))
for(i in 1:4){
  r <- hostpres[hostpres$yearsWithSoy==x1levels[i],"residual"]
...
#tiff("SROChostpres_histresid.tiff", width=1000, height = 1000, res=100)
layout(matrix(1:4,ncol=2,byrow=T))
...
#tiff(file="SROChostpres-residualvsfitted.tiff", width=550, height=500, res=100)
layout(matrix(1:4,ncol=2,byrow=T))
par(mar=c(4,4,3,2)+0.1)
plot(hostpres$yhat, hostpres$residual, xlab="model estimate",ylab="level 1 resid.",
...
resid.sd <- tapply(hostpres$residual, hostpres$yearsWithSoy, sd)
sigma
yrange <- c(min(resid.sd, sigma)-0.01, max(resid.sd,sigma)+0.01)
#tiff(file="SROChostpres-residvar.tiff", width=450,height=400,res=100)
...
x1levels <- levels(hostpres$yearsWithSoy)
...
##boundary for effect size is 0.327. Boundary for mean difference is 0.1
...
contsum.sim <- summarizecontrasts(contmat$meandiff, rope = rope.meandiff ) # contrasts from simulation with most-credible pars.
contsum.mcmc <- summarizecontrasts(contmatMCMC$meandiff, rope = rope.meandiff) # contrasts from MCMC chain
contsum.simeff <- summarizecontrasts(contmat$effectsize,rope=rope)
...
##Use contrasts from MCMC:
srocdf.pres <- hostpres
sroccontrasts.pres <- list(contrasts=contmatMCMC, contrastsummary = list(effectsize = contsum.mcmceff, meandiff = contsum.mcmc))
sroc.sim.pres <- list(x1means = meanofx1means.sim,x1HDI95 = HDI95.x1sim,
...
#save(srocdf.pres, sroccontrasts.pres, sroc.sim.pres, file="SROCobjectsForFig2_hostpres.RDATA")
